...
$.ajax({
  ...
  data: {
    key: 'MPYBZ-CR32H-GWQDU-WN234-XRMRZ-UKBPD',  // 这里要写你的KEY!!! API https://lbs.qq.com/dev/console/application/mine
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(121.79241, 29.822842, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function getWeibo() {
  fetch('https://weiboapi.hgez6.top/api').then(data => data.json()).then(data => {  // 这里要写上你的API!!!
...
function owoBig() {
  ...
  let observer = new MutationObserver(mutations => {
    ...
    for (let i = 0; i < mutations.length; i++) {
      ...
      // 禁用右键（手机端长按会出现右键菜单，为了体验给禁用掉 document.body.clientWidth <= 768）
      
      if (window.innerWidth < 768) owo_body.addEventListener('contextmenu', e => e.preventDefault());
      // 鼠标移入
...
function createtime1() {
  var grt = new Date("12/31/2021 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Fomalhaut🥝 %c 你正在访问大鸟🥝の小家",
...
// 边缘高亮色区别定义
map.set('redby', "rgb(241, 71, 71)");
map.set('orangeby', "rgb(241, 162, 71)");
map.set('yellowby', "rgb(241, 238, 71)")
map.set('purpleby', "rgb(179, 71, 241)");
map.set('blueby', "rgb(102, 204, 255)");
map.set('grayby', "rgb(226, 226, 226)");
map.set('greenby', "rgb(57, 197, 187)");
map.set('whitegrayby', "rgb(241, 241, 241)");
map.set('pinkby', "rgb(237, 112, 155)");
map.set('blackby', "rgb(0, 0, 0)");
map.set('darkblueby', "rgb(97, 100, 159)");
map.set('heoblueby', "rgb(66, 90, 239)");
...
class Cursor {
  ...
  create() {
    ...
    this.scr.innerHTML = `* {cursor: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 8 8' width='8px' height='8px'><circle cx='4' cy='4' r='4' opacity='1.0' fill='` + colorVal + `'/></svg>") 4 4, auto}`; //鼠标相关
  }
...
function createtime() {
  ...
  var start = new Date("12/31/2021 00:00:00"); // 旅行者1号开始计算的时间 第二个数字是月份
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("12/31/2021 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='/img/badge/FLY.svg' title='抓紧时间提高效率~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='/img/badge/SLEEP.svg' title='坚持10小时基准睡眠~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (window.localStorage.getItem("fpson") == undefined || window.localStorage.getItem("fpson") == "1") {
  ...
  var loop = function () {
    ...
    if (window.innerWidth < 768) {
        // js检测手机窗口宽度https://www.ruanyifeng.com/blog/2021/09/detecting-mobile-browser.html 
        document.getElementById("fps").innerHTML = `FPS:${fps}`;
      } else if(window.innerWidth >= 3800){
         document.getElementById("fps").innerHTML = `4K😍FPS:${fps} ${kd}`;
      }  
      else {
...
/* fps检测 end */
// 天气开关
if (window.localStorage.getItem("tqon") == undefined || window.localStorage.getItem("tqon") == "1") {
  document.getElementById("s-sticker").style = "display:table"
} else{document.getElementById("s-sticker").style = "display:none!important"}

/* 看板娘 start */
if (window.localStorage.getItem("kbn") == undefined || window.localStorage.getItem("kbn") == "1") {
const live2d_path = "https://unpkg.com/live2d-widget_hge@latest/"; 
// 封装异步加载资源的方法
...
if (screen.width >= 768) {
  ...
  // document.getElementById("waifu").style = "display:none!important" 不做任何事就行加上反而有Bug
}
/* 看板娘end */
/* 波浪 start */
if (window.localStorage.getItem("bolang") == undefined || window.localStorage.getItem("bolang") == "1") {
  document.documentElement.style.setProperty('--bolang-kg', null);
} else {
  document.documentElement.style.setProperty('--bolang-kg', "none");

  // document.getElementById("gentle-wave").style = "display:none!important"  //疑似会出现面板无法打开的情况
    // document.getElementById(".main-hero-waves-area").style = "display: none"
}
/* 波浪end */
...
function clearItem() {
  ...
  localStorage.removeItem('kbn');
  localStorage.removeItem('bolang');
  localStorage.removeItem('zttj');
  localStorage.removeItem('ztdx');
  localStorage.removeItem('Colorby');
  localStorage.removeItem('wzkjtj');
...
// 设置边缘高亮色
if (localStorage.getItem("Colorby") == undefined) {
  localStorage.setItem("Colorby", "greenby");
}
setColorby(localStorage.getItem("Colorby"));
function setColorby(c) {
  localStorage.setItem("Colorby", c);
  // 设置一个带有透明度的主题色，用于菜单栏的悬浮颜色
  var theme_colorby = map.get(c);
  if(c=="blackby"){
  document.documentElement.style.setProperty("--card-hover-box-shadow", "0 3px 8px 6px rgba(7, 17, 27, .09)");  
  } else {
  var high_colorby = "0 3px 8px 6px rgba" + theme_colorby.substring(3, theme_colorby.length - 1) + ", 0.6)";
  document.documentElement.style.setProperty("--card-hover-box-shadow", high_colorby);
  }
...
// 气泡开关
if (localStorage.getItem("bubble") == undefined) {
  localStorage.setItem("bubble", "block");
}
document.getElementById("bubble").style.display = localStorage.getItem("bubble");
function setBubble() {
  if (document.getElementById("bubbleSet").checked) {
    document.getElementById("bubble").style.display = "block";
    localStorage.setItem("bubble", "block");
  } else {
    document.getElementById("bubble").style.display = "none";
    localStorage.setItem("bubble", "none");
  }
}
...
//天气开关
if (localStorage.getItem("tqon") == undefined) {
  localStorage.setItem("tqon", "1");
}
function tqsw() {
  if (document.getElementById("tqon").checked) {
    localStorage.setItem("tqon", "1");
  } else {
    localStorage.setItem("tqon", "0");
  }
  setTimeout(reload, 600);
}
...
// 看板娘开关
if (localStorage.getItem("kbn") == undefined) {
  localStorage.setItem("kbn", "1");
}
function kbnsw() {
  if (document.getElementById("kbn").checked) {
    localStorage.setItem("kbn", "1");
  } else {
    localStorage.setItem("kbn", "0");
  }
  setTimeout(reload, 600);
}

// 波浪开关
if (localStorage.getItem("bolang") == undefined) {
  localStorage.setItem("bolang", "1");
}
function bolangsw() {
  if (document.getElementById("bolang").checked) {
    localStorage.setItem("bolang", "1");
  } else {
    localStorage.setItem("bolang", "0");
  }
  setTimeout(reload, 600);
}
//字体粗细大小记忆
...
//       this.$notify({
//         title: "欢迎移动端用户🍨",
//         message: "你为您进行移动端优化😛，欢迎来访又是充满希望的一天😎",
//         position: 'top-left',
//         offset: 50,
//         showClose: true,
//         type: "success",
...
var zttjv=parseInt(localStorage.getItem("zttj"));
var ztdxv=localStorage.getItem("ztdx");
var wzkjtjv=localStorage.getItem("wzkjtj");
// if (localStorage.getItem("zttj") == undefined){
//   zttjv=0;
// }
if (zttjv>=100 && zttjv<=999) {
  document.documentElement.style.setProperty('--zt-cuxitj', zttjv);
} else if(window.innerWidth < 768){
  document.documentElement.style.setProperty('--zt-cuxitj', "750");
}
// else {
//   zttjv=0;
// }
...
//   zttjv=0;
// }
// if (localStorage.getItem("ztdx") == undefined){
//   zttjv=0;
// }
if (ztdxv>=10 && ztdxv<=60) {
  document.documentElement.style.setProperty('--zt-dx', ztdxv+"px");
} else if(window.innerWidth < 768){
  document.documentElement.style.setProperty('--zt-dx', "15px");
}

if (wzkjtjv>=60 && wzkjtjv<=115) {
  document.documentElement.style.setProperty('--wzkg-width', wzkjtjv+"%");
} else if(window.innerWidth > 3000){
  document.documentElement.style.setProperty('--wzkg-width', "100%");
}
...
// 更换背景(自己的代码)随机背景
var bannerimg =   [  
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/1.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/3.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/4.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/5.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/6.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/7.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/8.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/9.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/11.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/12.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/13.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/14.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/15.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/16.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/17.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/18.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/19.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/20.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/22.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/23.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/25.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/29.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/30.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/31.jpg)"
  ];  
var bannerindex =Math.floor(Math.random() * bannerimg.length);
var bannerimgd =   [  
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/10.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/27.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/21.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/28.jpg)",
  "url(https://yun.hgez6.top/api/raw/?path=/picbed/26.jpg)"
  ];  
var bannerindexd =Math.floor(Math.random() * bannerimgd.length);
if (localStorage.getItem("blogbg") != undefined) {
  setBg(localStorage.getItem("blogbg"));
} else {
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: ${bannerimg[bannerindex]};
    --darkmode-bg: ${bannerimgd[bannerindexd]};
    --mobileday-bg: ${bannerimg[bannerindex]};
    --mobilenight-bg: ${bannerimgd[bannerindexd]};
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
// 调节字体粗细函数
function getPx1() {
  debounce(getPx1_, 300);
}

function getPx1_() {
  
  var wenzhangfontcuxi = document.getElementById("zttj").value;
  
    // 有效的数据
  if(wenzhangfontcuxi>=100 && wenzhangfontcuxi<=999){
    localStorage.setItem("zttj", wenzhangfontcuxi);
    document.documentElement.style.setProperty('--zt-cuxitj', wenzhangfontcuxi);
    document.getElementById("article-container").style.fontWeight=wenzhangfontcuxi;
...
// 调节字体大小函数
function getPx2() {
  debounce(getPx2_, 300);
}

function getPx2_() {
  

  var wenzhangfontdx = parseInt(document.getElementById("ztdx").value)
    // 有效的数据
  if(wenzhangfontdx>=10 && wenzhangfontdx<=60){
    var wenzhangfontdx2 = document.getElementById("ztdx").value+"px"  //调节大小一定要加px
    localStorage.setItem("ztdx", wenzhangfontdx);
    document.documentElement.style.setProperty('--zt-dx', wenzhangfontdx2);
    // document.getElementById("article-container").style.fontSize=wenzhangfontdx2;
...
// 调节文章宽度函数
function getPx3() {
  debounce(getPx3_, 300);
}

function getPx3_() {
  

  var wenzhangkjdx = parseInt(document.getElementById("wzkjtj").value)
    // 有效的数据
  if(wenzhangkjdx>=60 && wenzhangkjdx<=115){
    var wenzhangkjdx2 = document.getElementById("wzkjtj").value+"%"  //调节大小一定要加px
    localStorage.setItem("wzkjtj", wenzhangkjdx);
    document.documentElement.style.setProperty('--wzkg-width', wenzhangkjdx2);
    // document.getElementById("article-container").style.fontSize=wenzhangfontdx2;
...
if (localStorage.getItem("aside") == "1") {
  document.documentElement.style.setProperty('--layout-justify-content', "unset");
  document.documentElement.style.setProperty('--aside-content-display', "block");
} else {
  document.documentElement.style.setProperty('--layout-justify-content', "center");
  document.documentElement.style.setProperty('--aside-content-display', "none");
}
function setAside() {
  document.getElementById("asideSet").checked ? (localStorage.setItem("aside", "1"),
  document.documentElement.style.setProperty('--layout-justify-content', "unset"),
  document.documentElement.style.setProperty('--aside-content-display', "block")) : (localStorage.setItem("aside", "0"),
  document.documentElement.style.setProperty('--layout-justify-content', "center"),document.documentElement.style.setProperty('--aside-content-display', "none"))
}
//侧栏左右切换
if ((localStorage.getItem("asidePos") !="1" && localStorage.getItem("asidePos") !="0") || localStorage.getItem("asidePos")==undefined) {
  localStorage.setItem("asidePos", "1");
  document.getElementById("asidePosSet").checked = true;
}
if (localStorage.getItem("asidePos") == "1") {
  document.documentElement.style.setProperty('--first-child-order', "2");
  document.documentElement.style.setProperty('--recent-post-item-margin', "0px 0px 20px 1%");
} else {
  document.documentElement.style.setProperty('--first-child-order', "0");
  document.documentElement.style.setProperty('--recent-post-item-margin', "0px 1% 20px 0px");
}
function setAsidePos() {
  document.getElementById("asidePosSet").checked ? (localStorage.setItem("asidePos", "1"),
  document.documentElement.style.setProperty('--first-child-order', "2"),
  document.documentElement.style.setProperty('--recent-post-item-margin', "0px 0px 20px 1%")) : (localStorage.setItem("asidePos", "0"),
  document.documentElement.style.setProperty('--first-child-order', "0"),
  document.documentElement.style.setProperty('--recent-post-item-margin', "0px 1% 20px 0px"))
}
//顶栏显隐有bug
...
<div class="content" style="display:flex">
  ...
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 气泡特效(默认开) </div><input type="checkbox" id="bubbleSet" onclick="setBubble()">
</div>
...
<div class="content" style="display:flex">
  ...
  <div class="content-text" style="font-weight:bold; padding-left:20px"> 小部件栏 (默认开) </div><input type="checkbox" id="rightSideSet" onclick="toggleRightside()">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 看板娘 (默认开-仅PC端刷新生效) </div><input type="checkbox" id="kbn" onclick="kbnsw()">
</div>
...
<div class="content" style="display:flex">
  ...
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 波浪特效 (仅白天默认开) </div><input type="checkbox" id="bolang" onclick="bolangsw()">
</div>
<div class="content" style="display:flex">
  ...
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 侧栏位置 (默认左边) </div><input type="checkbox" id="asidePosSet" onclick="setAsidePos()">
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 天气开关 (自动刷新) </div><input type="checkbox" id="tqon" onclick="tqsw()">
</div>


<input type="text" id="wzkjtj" size="50%" maxlength="10" placeholder="74％默认100％舒适115％最大，当前值：${wzkjtjv}">&nbsp<button type="button" onclick="getPx3()" style="background:var(--theme-color);width:20%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">调节文章宽度</button>
<h2>二、字体设置</h2>
{% note warning modern %}非商免字体未经授权只能个人使用。本站为完全非商业、非盈利性质的网站，平时用于个人学习交流，如有侵权请联系站长删除，谢谢！ —— 致版权方{% endnote %}

<input type="text" id="zttj" size="50%" maxlength="10" placeholder="400默认700加粗999最粗，当前值：${zttjv}">&nbsp<button type="button" onclick="getPx1()" style="background:var(--theme-color);width:20%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">调节粗细</button>
...
<a class="swf" id="swf_HarmonyOS" href="javascript:;" rel="noopener external nofollow" style="font-family:'HarmonyOS'!important;color:black" onclick="setFont('HarmonyOS')">鸿蒙字体</a>
...
<input type="text" id="ztdx" size="50%" maxlength="10" placeholder="PC端：25默认，手机端：15默认，当前值：${ztdxv}">&nbsp<button type="button" onclick="getPx2()" style="background:var(--theme-color);width:20%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">调节大小</button>
...
<h2>四、边缘高亮色设置</h2>
<div class="contents" style="display:flex"><input type="radio" id="blackby" name="colorsby" value=" "
        onclick="setColorby('blackby')"><input type="radio" id="orangeby" name="colorsby" value=" "
        onclick="setColorby('orangeby')"><input type="radio" id="greenby" name="colorsby" value=" "
        onclick="setColorby('greenby')" checked="checked"><input type="radio" id="blueby" name="colorsby" value=" "
        onclick="setColorby('blueby')"><input type="radio" id="purpleby" name="colorsby" value=" "
        onclick="setColorby('purpleby')"><input type="radio" id="pinkby" name="colorsby" value=" "
        onclick="setColorby('pinkby')" ><input type="radio" id="redby" name="colorsby" value=" "
        onclick="setColorby('redby')"><input type="radio" id="heoblueby" name="colorsby" value=" "
        onclick="setColorby('heoblueby')"><input type="radio" id="darkblueby" name="colorsby" value=" "
        onclick="setColorby('darkblueby')"><input type="radio" id="yellowby" name="colorsby" value=" "
        onclick="setColorby('yellowby')"></div> 
...
`;
  ...
  $("#" + localStorage.getItem("Colorby")).attr("checked", true); //这两个都用了any运算符冲突了所以重新定义一个颜色Map
  ...
  if (localStorage.getItem("tqon") == "1") {
    document.getElementById("tqon").checked = true;
  } else {
    document.getElementById("tqon").checked = false;
  }
  //看板娘初始化
  if (localStorage.getItem("kbn") == "1") {
    document.getElementById("kbn").checked = true;
  } else {
    document.getElementById("kbn").checked = false;
  }
    //波浪初始化
  if (localStorage.getItem("bolang") == "1") {
    document.getElementById("bolang").checked = true;
  } else {
    document.getElementById("bolang").checked = false;
  }
  ...
  //气泡默认开以及记忆功能初始化
  if (localStorage.getItem("bubble") == "block") {
    document.getElementById("bubbleSet").checked = true;
  } else if (localStorage.getItem("bubble") == "none") {
    document.getElementById("bubbleSet").checked = false;
  }
  if (localStorage.getItem("aside") == "1") {
    document.getElementById("asideSet").checked = true;
  } else 
